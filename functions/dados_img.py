...
def rotular_imagem(dados):
	...
	for rotulo in os.listdir(dados):
		...
		for imagem in os.listdir(pacote_dados):
			...
			if np.any(img == None):
				continue
			rotulo_imagem.append((np.array(img, dtype=np.float16), int(rotulo)))
...
def dados_img():
	...
	imagem_treino = imagens[:int(5/6*len(imagens))]
	print("Itens em imagem_treino", len(imagem_treino))
	with open("modelo/treino/imagem_treino", "wb") as f:
		pickle.dump(imagem_treino, f)
	del imagem_treino

	rotulo_treino = rotulos[:int(5/6*len(rotulos))]
	print("Itens em rotulo_treino", len(rotulo_treino))
	with open("modelo/treino/rotulo_treino", "wb") as f:
		pickle.dump(rotulo_treino, f)
	del rotulo_treino

	imagem_teste = imagens[int(5/6*len(imagens)):]
	valor_imagem = imagem_teste[:int(0.5*len(imagem_teste))]
	imagem_teste = imagem_teste[int(0.5*len(imagem_teste)):]
	print("Itens em imagem_teste", len(imagem_teste))
	with open("modelo/treino/imagem_teste", "wb") as f:
		pickle.dump(imagem_teste, f)
	del imagem_teste

	rotulo_teste = rotulos[int(5/6*len(rotulos)):]
	valor_rotulo = rotulo_teste[:int(0.5*len(rotulo_teste))]
	rotulo_teste = rotulo_teste[int(0.5*len(rotulo_teste)):]
