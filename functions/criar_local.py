...
def cria_mascara(shape, img):
	...
	mascara = cv2.line(mascara,(shape[35,0],shape[35,1]),(shape[30,0],shape[30,1]),255,1)
	mascara = cv2.line(mascara,(shape[41,0],shape[41,1]),(shape[36,0],shape[36,1]),255,1)
	mascara = cv2.line(mascara,(shape[47,0],shape[47,1]),(shape[42,0],shape[42,1]),255,1)
	mascara = cv2.line(mascara,(shape[67,0],shape[67,1]),(shape[60,0],shape[60,1]),255,1)
	mascara = cv2.line(mascara,(shape[60,0],shape[60,1]),(shape[48,0],shape[48,1]),255,1)
	mascara = cv2.line(mascara,(shape[64,0],shape[64,1]),(shape[54,0],shape[54,1]),255,1)	
...
def criar_local():
	...
	i=0
	DATASET = '../dados/'
	FACES = input("Digite o nome da pasta que contem as imagens para conversão: ")
	FACES = ("../"+FACES+"/")
	...
	for folder in folders:
		...
		for image in os.listdir(FACES+folder):
			...
			if img is not None: 
				...
				if len(faces) > 0:
					for rosto in faces:
						...
						cv2.imwrite(DATASET+folder+image, mascara_alinhada)	
						#print(str(i))			
						i+=1
			...
			if cv2.waitKey(1) == ord('q'):
				break			
	print("Ao total foram convertidas "+str(i)+" imagens.")
